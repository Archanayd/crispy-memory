#enter fungi folder, keep all_programs and main hmm resultsfolder one directory out
cat make_dir.txt|xargs mkdir
cp ../all_programs/rename_hmm.pl  fungi_hmm_res.txt ids_withspnames 1_rename_hmm/
cd 1_rename_hmm/
mv fungi_hmm_res.txt hmm_res_in.txt
perl rename_hmm.pl 
cd ..
...
./z.sh
cd ../
cp -r ../all_programs/multifeature_plots ./
...
mkdir fungi
mkdir fungi/fasta_without_tags fungi/domtblout
cp ../fungi_update/4_fasta/* fungi/fasta_without_tags/
rm -rf fungi/fasta_without_tags/*.txt fungi/fasta_without_tags/*.pl
./hmmscan_script_fungi.sh
cd fungi
...
cp ../domain_arrangemnts/fungi/create_table/tables/*.html 12_add_anchors/tables/
cd 12_add_anchors/
...
cd 13_add_headers/
cp ../../all_programs/add_header-fasta_fungi.pl ../../all_programs/add_header-ss_fungi.pl ./
...
ls *.html>y
cp y x
gsed -i 's/.*/perl add_header-fasta_fungi.pl/' x
paste x y>z.sh
chmod +x z.sh 
./z.sh
mv result/* fasta/
...
ls *.html>y
cp y x
gsed -i 's/.*/perl add_header-ss_fungi.pl/' x
paste x y>z.sh
./z.sh
mv result/* clustal/
rm -rf *.html
